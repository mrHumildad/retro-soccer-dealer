  ...
  return (
    <div className="market-container">
      <div className="market-header-row">
        <h2 className="market-header">Can buy {buy4month} of {poolSize}</h2>
        <button className={`button ${canBuyBuy4month ? 'button-secondary' : 'button-disabled'}`} disabled={!canBuyBuy4month} onClick={onBuyBuy4month} title={canBuyBuy4month ? `Buy buy for ${formatValue(buy4monthPrice)}` : 'Not enough money'}>
          <span className="button-top">+ BUY</span>
          <span className="button-bottom">{formatValue(buy4monthPrice)}</span>
        </button>
        <button className={`button ${canBuyPoolSize ? 'button-secondary' : 'button-disabled'}`} disabled={!canBuyPoolSize} onClick={onBuyPoolSize} title={canBuyPoolSize ? `Buy pool size for ${formatValue(poolSizePrice)}` : 'Not enough money'}>
